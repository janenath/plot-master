...
export default class Plot extends Component {
	...
	newPlot = event => {
		...
		if (
			Number(this.state.height) > 25 ||
			Number(this.state.width) > 25 ||
			Number(this.state.height) < 0 || Number(this.state.height) < 0
		) {
			alert(
				'Plot is too large or too negative for our database! try values below 25 ft but hopefully above zero'
			);
			this.setState({
				width: 0,
				height: 0,
